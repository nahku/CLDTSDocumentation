...
The modified graph represents the syntax of the extracted sub-language.
To make this syntax usable, lastly the syntax must be output in the same format as the original syntax.
\begin{figure}[H]
...
The four expressions are the expression token followed by an productions list. Grammar expression has a special case without a productions list because the production rule
\begin{verbatim}
<null>   ::= 
...
Starting with the temporary start symbol, the graph is generated recursively (described in chapter \ref{sec:ImplementationGraphGeneration}).

The following example shows a production rule and an abstract representation of the resulting list of children belonging to the node. Each production alternative is embedded in the children list of the node. %This is relevant for checking whether a nonterminal is terminating.
...
The flow chart in figure \ref{fig:ConceptMaintainingComments} shows the process of matching comment blocks, that are consecutive comment lines,  to rules.
...
In the control file, for each nonterminal symbol, that will be modified, its name, rule type and the indexes of the productions that should be blocked are listed.
...
The automated parser generator for the \ac{TPTP} syntax \cite{VS06} takes a \ac{TPTP} syntax file as input and creates a Lex and Yacc file corresponding to the specification in the input syntax. It is used for validating that an extracted sub-syntax is compatible with the original \ac{TPTP} syntax.\\
...
The graphical user interface is built using PyQt. Advantages of PyQT are that it offers a treeview that is used for displaying the grammar. In comparison to the standard Python tool kit Tkinter \cite{Tkinter} PyQt offers checkboxes in treeviews that are used for selecting blocked productions.
...
In addition to the GUI, \ac{Synplifier} offers a command-line interface to provide the means for convenient automation of sub-syntax extraction.
...
To count rules of the original \ac{TPTP} syntax (including non-reachable and nonterminating symbols) a script has been developed.
