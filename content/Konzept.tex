...
The following example \ref{lst:ConceptGraphGeneration} shows a production rule and the resulting list of children belonging to the node. Each production alternative(REFERENCE) has its own list of children. %This is relevant for checking whether a nonterminal is terminating.
...
For that, t
deleting the productions from the productions list and also the corresponding elements from the childrens list.
...
This can be done by generating a new grammar graph starting from desired the start symbol.
When generating the new grammar graph, starting from the start symbol all reachable parts of the grammar will be added to the grammar graph (see section \ref{sec:ConceptGraphGeneration}). All nodes that are not part of the new grammar graph are not reachable and can be removed.
...
Providing the option of importing the \ac{TPTP} syntax from the project website in addition to importing a \ac{TPTP} syntax file from storage ensures the possibility of getting the latest version of the \ac{TPTP} syntax.
