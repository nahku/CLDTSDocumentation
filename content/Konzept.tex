...
\ac{Synplifier} should meet the following requirements:\\
\subsubsection{[R1]}\label{R1}


\ac{Synplifier} MUST provide a GUI with the following functionalities:
...
\ac{Synplifier} MUST provide the functionality for the user to import a \ac{TPTP} syntax file into the GUI by specifying a \ac{TPTP} syntax file path and a desired start symbol. The imported syntax MUST be displayed in the GUI.
...
console interface help menu

-extract sub-syntax

R
\ac{Synplifier} MUST provide the functionality to generate a sub-syntax based on a control file.

nonfunctional requirements
\ac{Synplifier} has a GUI that is the interface between \ac{Synplifier} and a user. Hence, the user communicates with \ac{Synplifier} via the GUI.
%The user is able to import a syntax file. After the syntax file is imported, it should is displayed. 
...
%\ac{Synplifier} can also generate a control file listing blocked productions and the start symbol.
...
The following example shows a production rule and the resulting list of children belonging to the node. Each production alternative has its own list of children. %This is relevant for checking whether a non-terminal is terminating.
\begin{verbatim}
Production rule:
