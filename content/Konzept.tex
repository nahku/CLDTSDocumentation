...
Rules are counted for comparing syntax sizes and for generating statistics out of it. The results are described in chapter \ref{sec:Validation}.

\subsection{Counting rules in a grammar graph}\label{sec:ConceptCountR}

For comparing syntax sizes, rules and productions are counted.
Rules are counted by counting nodes in the grammar graph. Productions are counted by traversing the nodes in the grammar graph.\\
In addition only grammar rules are counted as well.
The number of rules is printed in the console after the import and after the sub-syntax generation. \\
Since the grammar graph only contains reachable and terminating symbols, not all rules of the \ac{TPTP} syntax can be counted.

\subsection{Counting rules in the original syntax}\label{sec:ConceptCountR}

To count rules of the full \ac{TPTP} syntax (including non reachable and terminating symbols) a separate script is created which uses the grammar list produced by the parser and counts based on this grammar list. Counting rules based on the full \ac{TPTP} syntax gives a better overview when comparing sub-syntaxes. 
