...
The capitalized keyword are to be interpredet as described in \cite{Bradner.1997}.
The tool should meet the following requirements:\\
\subsubsection{[R1]}\label{R1}
\subsubsection{[R2]}\label{R2}
The user is able to import a \ac{TPTP} syntax file.
\subsubsection{[R3]}\label{R2}
The tool MUST be able to display \ac{TPTP} syntax file.
\subsubsection{[R3]}\label{R3}
\subsubsection{[R4]}\label{R4}
\subsubsection{[R5]}\label{R5}
...
Within the software tool a syntax is represented by a grammar graph.
...
The user also has the option of generating a control file based on his GUI selection. This is particularly helpful if the users is reducing the same grammar multiple times. This way he can use the import control file option later instead of selecting the blocked productions in the GUI tool.
