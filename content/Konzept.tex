...
The goal that the parser component should fulfill is to take the tokens from the lexer as input and create a data structure that represents the structure of the \ac{TPTP} language grammar specification sign.
...
To build the representative data structure, represenations of todo... have to be defined.
The following section describes the %data structures and
data types representing the \ac{TPTP} language grammar specification that are being used and created by the parser in the parsing process.

Atomary data types

\subsubsection{Terminal Symbol}
A terminal symbol has name/value? that is a sequence of characters.
-Production Property
...
A grammar expression consists of the nonterminal name, a production list and a position.
The position denotes where in
\subsubsection{Token Expression}
Analogue to grammar expression
\subsubsection{Strict  Expression}

\subsubsection{Macro Expression}
