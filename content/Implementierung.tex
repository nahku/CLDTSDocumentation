...
-comment block \textunderscore \textunderscore str \textunderscore \textunderscore method creates string

\subsubsection{\textit{PRODUCTIONS\textunderscore LIST} string representation creation}
-list of productions
-productions are separated by " | "
-and str of individual productions called
-reference to uml

Algorithm \ref{alg:OutputCreateProductionsListString}

-counter to check last production and do not print alternative symbol after
...
-order
-production list consists of production, xor productions list or production element

-else case (for production element and xor productions list)
...
- rules are in string format and cohered 
- create one string out of cohered strings
- save string to local storage

- initialise visited 
- create empty print list and empty print string
- generate print list using create print list
- print list is not in the same order as the orginal tptp grammar, based on the position that has been stored in each print list entry, the print list is sorted
- every element of the print list except of the first one which is the general start symbol that has been added but is not a part of the tptp grammar is added to the print string separated by a new line
- a text document with the filename is opened and the print string is written to the file

\begin{figure}[H]
\centering
\includegraphics[width=0.3\textwidth]{images/Implementation_saveorderedrules.pdf}
