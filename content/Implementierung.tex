...
\caption{Split comment block by top of page line}
\label{fig:ImplementationMaintainingCommentSplitBTopOfPage}
\end{figure}
...
It returns a list of \textit{COMMENT\textunderscore BLOCK} objects.\\
In the following if-clause it is checked, how many \textit{COMMENT\textunderscore BLOCK} object are returned.\\
...
In line 5 it is checked if the \textit{COMMENT\textunderscore BLOCK} is not at the end of the syntax file. If that is the case it should be associated with the rule after.\\
For that the next rule (rule after the \textit{COMMENT\textunderscore BLOCK}) is obtained, by indexing the next element of the \textit{rules\textunderscore list}.
Then, the \textit{COMMENT\textunderscore BLOCK} is appended to the node, specified by the rule, using the \textit{append\textunderscore comments\textunderscore to\textunderscore node} method. This method is described in the following ?todo?.\\
...
The procedure of adding the \textit{COMMENT\textunderscore BLOCK} to the desired node is essentially the same as described before. The difference lies in that the previous rule is adressed (line 9).\\
If splitting the original \textit{COMMENT\textunderscore BLOCK} by top of page lines results in two \textit{COMMENT\textunderscore BLOCK}s the first is associated with the rule before and the second with the rule after (lines 14 - 15, 21 - 22).
