...
Algorithm \ref{alg:buildNodesDictionary} shows how the nodes dictionary is build. Iterating the rules list that the parser outputs, if the rule is not a comment block, a new dictionary item is created that consists of the node key rule name and rule type and the NTNode as value.

todo
-multiple rules can correspond to one node, example
-to not overwrite node, check if key already exists
-in that case append productions list
-keep first position
