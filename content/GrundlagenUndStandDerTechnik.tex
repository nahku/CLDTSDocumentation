...
This section first introduces the terms alphabet, string and language.
- also regular expression, finite automata and grammars

\subsection{Alphabet}
An alphabet is a finite, non-empty set of symbols usually represented by $\sum$. An example is the binary alphabet $\sum = \{0,1\}$.

\subsection{String}
A string also called word is a finite sequence of symbols from some alphabet. For example the string $101$ is a string from the binary alphabet $\sum = \{0,1\}$.

\subsection{Language}
A formal language is a set of words over an alphabet. If $\sum$ is an alphabet, then $L(\sum)$ is a language over $\sum$. \cite{AutomataTheory.2007}
...
- states and transitions in response to inputs
- states are represented by circles
- start state
- final/accepting states
- deterministic vs nondeterministic
- recognize words of a language

- example below shows automation that excepts word consistsing of 1, arbitrary amount of zeros and a 1, regular expression: 10*1
...
- can describe the same "patterns" that can also be represented by a finite automaton
...
The grammatically description of a language consists of four components:
