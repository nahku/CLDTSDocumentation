...
-construction of a parse tree for input string beginning at the leaves bottom working up to the root (top)
-one class shift reduce parsing
-lr grammar?
-bottom up parsing -> reducing a string to the start symbol of a grammar (construct deviation in reverse)
...
-a stack holds grammar symbols, input buffer holds rest of string to be parsed
-handle appears at top of the stack
-during left ot right scan of input, parser shifts zero or more input symbols on stack until it is ready to reduce string on top of the stack, ti the reduces string to the head of the appropriate production
-parser repeats cycle until error is detected or stack contains start symbol and input is empty
shift reduce parser steps:
shift: shipt next input symbol on top of stack
reduce: right end of sting to be reduced must be at top of stack, locate left end of thring within the stack and decide with what nonterminal to replace the string
accept: successful
error: syntax error

conflicts:
-shift reduce does not work for all context free gramamrs
-if parser can not decide whether to shift or reduce (shift/reduce conflict), or can not decide which reduction to make (reduce/reduce conflict)
